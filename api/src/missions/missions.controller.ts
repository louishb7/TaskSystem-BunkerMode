...
export class MissionsController {
  ...
  async soldierBoard(@Req() request: AuthenticatedRequest) {
    ...
    return {
      missions: board.action_missions.map((mission) =>
        toMissionResponse(mission, user),
      ),
      daily_missions: board.daily_missions.map((mission) =>
        toMissionResponse(mission, user),
      ),
    };
  }
  ...
  async missionHistory(
    @Req() request: AuthenticatedRequest,
    @Param("id") id: string,
  ) {
    const events = await this.missionsService.missionHistory(
      missionId(id),
      request.currentUser!,
    );
